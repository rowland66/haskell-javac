...
data Environment = Environment {variableTypeMap :: Map.Map P.SimpleName P2.Type
...
createMethodEnvironment typeData clazz@(P2.NewClazz2 _ _ className _ _ _) (P2.NewMethod _ _ params _ _ _) =
  let env = Map.insert P.createNameThis (P2.L className) $ foldr (\(P2.NewParameter _ tp nm) env -> Map.insert nm (P2.L tp) env) Map.empty params
...
createConstructorEnvironmentRight typeData (P2.NewClazz2 _ _ className _ _ _) (P2.NewMethod _ _ params _ _ _) =
  let env = foldr (\(P2.NewParameter _ tp nm) env -> Map.insert nm (P2.L tp) env) Map.empty params
      (_, envPos') = foldl (\(i, env) (P2.NewParameter _ _ nm) -> (i+1, Map.insert nm i env)) (1, Map.empty) params
      envPos = Map.insert P.createNameThis 0 envPos' in
  Environment {variableTypeMap=env, variablePositionMap=envPos, typeData=typeData}
...
createConstructorEnvironmentLeft typeData (P2.NewClazz2 _  _ className _ _ _) =
  let env = Map.insert P.createNameThis (P2.L className) Map.empty
      envPos = Map.insert P.createNameThis 0 Map.empty in
  Environment {variableTypeMap=env, variablePositionMap=envPos, typeData=typeData}

(!?) :: Environment -> P.SimpleName -> Maybe (P2.Type,Int)
